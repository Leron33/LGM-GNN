# -*- coding: utf-8 -*-
"""
Created on Tue May 21 23:04:24 2024
...
def run(n,p,S,Itera):
    ...
    for si, s in enumerate(S):
        for itera in range(Itera):
            ...
            result = FAQ(G1,G2)
            faq[itera,si] = sum((result[y[0]]==y[1]).float())/n
            
    
            
    _,faq = torch.std_mean(faq,dim=0,unbiased=False)

    
    S = ', '.join(str(round(i, 4)) for i in S)
    faq = ', '.join(str(round(i, 4)) for i in (faq).tolist())
    
    torch.set_printoptions(precision=4)
    print(f'Parameters: n={n}, p={p}')
    print('Accuracy')
    print('s = '.ljust(10), S)
    print('faq = '.ljust(10), faq)
