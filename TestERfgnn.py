...
from models.FGNN import FGNN as FGNN
...
device = "cpu"
model = FGNN(64, args.hid, args.num_layers).to(device)

def sinkhorn(src, itera=5):
...
def FGNNq(G1,G2):
    d1 = torch.nn.functional.one_hot(torch.sum(G1,dim=1).long(),64).float()
    d2 = torch.nn.functional.one_hot(torch.sum(G2,dim=1).long(),64).float()
    y1 = model(d1,G1.to_sparse().indices())
    y2 = model(d2,G2.to_sparse().indices())
    
    W = sinkhorn(y1 @ y2.transpose(-1, -2))
    return W
