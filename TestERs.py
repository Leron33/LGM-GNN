...
def test(test_dataset):
    ...
    total_correct = torch.zeros(args.num_layers+1)
    total_c = 0
    total_node = 0
    for data in test_dataset:
        ...
        W,L= model(G1, G2, adj1, adj2,10)
        
        for li in range(args.num_layers+1):
            correct = model.acc(L[li], Y)
            total_correct[li] += correct
        total_node += num_nodes
    return total_correct/total_node, total_c/total_node

def run(n,p,S,Itera):
    
    gnn = torch.zeros(args.num_layers+1,len(S))
